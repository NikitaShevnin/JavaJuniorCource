...
/**
 * TODO ЗАДАНИЕ 2. Переработать метод балансировки корзины товаров cardBalancing() с использованием Stream API
 */
...
public class Program {
    ...
    */
    ...
    static <T extends Food> void cardBalancing() {
        cardBalancing(null);
    }

    static <T extends Food> void cardBalancing(Sem1.ClassWork.task2.Cart<T> cart) {
        List<T> foodstuffs = (List<T>) cart.getFoodstuffs();

        // Группируем продукты по типу
        Map<Class<? extends Food>, List<T>> groupedFoodstuffs = foodstuffs.stream()
                .collect(Collectors.groupingBy(T::getClass));

        // Балансируем каждую группу товаров
        groupedFoodstuffs.forEach((clazz, list) -> {
            UMarket market = cart.getMarket();
            List<Food> balancedList = market.balanceFoodstuffs(list);

            // Заменяем исходные продукты в корзине на сбалансированные
            foodstuffs.removeAll(list);
            foodstuffs.addAll((List<T>) balancedList); // Параметризованный тип

            System.out.println("Продукты получили сбалансированное количество.");
        });
    }
